export const PROFESSION_OPTIONS = Array.from(new Set([
  "Software Professional",
  "Teaching / Academician",
  "Executive",
  "Doctor",
  "Manager",
  "Professor / Lecturer",
  "Officer",
  "Human Resources Professional",
  "Supervisor",
  "Administrative Professional",
  "Clerk",
  "Secretary / Front Office",
  "Agriculture & Farming Professional",
  "Horticulturist",
  "Pilot",
  "Air Hostess / Flight Attendant",
  "Airline Professional",
  "Architect",
  "Interior Designer",
  "Chartered Accountant",
  "Company Secretary",
  "Accounts / Finance Professional",
  "Banking Professional",
  "Auditor",
  "Financial Accountant",
  "Financial Analyst / Planning",
  "Investment Professional",
  "Fashion Designer",
  "Beautician",
  "Hair Stylist",
  "Jewellery Designer",
  "Designer (Others)",
  "Makeup Artist",
  "BPO / KPO / ITeS Professional",
  "Customer Service Professional",
  "Civil Services (IAS / IPS / IRS / IES / IFS)",
  "Analyst",
  "Consultant",
  "Corporate Communication",
  "Corporate Planning",
  "Marketing Professional",
  "Operations Management",
  "Sales Professional",
  "Senior Manager / Manager",
  "Subject Matter Expert",
  "Business Development Professional",
  "Content Writer",
  "Army",
  "Navy",
  "Defence Services (Others)",
  "Air Force",
  "Paramilitary",
  "Dentist",
  "Surgeon",
  "Veterinary Doctor",
  "Education Professional",
  "Training Professional",
  "Research Assistant",
  "Research Scholar",
  "Civil Engineer",
  "Electronics / Telecom Engineer",
  "Mechanical / Production Engineer",
  "Quality Assurance Engineer - Non IT",
  "Engineer - Non IT",
  "Designer",
  "Product Manager - Non IT",
  "Project Manager - Non IT",
  "Hotel / Hospitality Professional",
  "Restaurant / Catering Professional",
  "Chef / Cook",
  "Hardware Professional",
  "Product Manager",
  "Project Manager",
  "Program Manager",
  "Animator",
  "Cyber / Network Security",
  "UI / UX Designer",
  "Web / Graphic Designer",
  "Software Consultant",
  "Data Analyst",
  "Data Scientist",
  "Network Engineer",
  "Quality Assurance Engineer",
  "Lawyer & Legal Professional",
  "Legal Assistant",
  "Journalist",
  "Media Professional",
  "Entertainment Professional",
  "Event Management Professional",
  "Advertising / PR Professional",
  "Actor / Model",
  "Artist",
  "Health Care Professional",
  "Paramedical Professional",
  "Nurse",
  "Pharmacist",
  "Physiotherapist",
  "Psychologist",
  "Therapist",
  "Medical Transcriptionist",
  "Dietician / Nutritionist",
  "Lab Technician",
  "Medical Representative",
  "Mariner / Merchant Navy",
  "Sailor",
  "Technician",
  "Arts & Craftsman",
  "Librarian",
  "Business Owner / Entrepreneur",
  "Retired",
  "Transportation / Logistics Professional",
  "Agent / Broker / Trader",
  "Contractor",
  "Fitness Professional",
  "Security Professional",
  "Social Worker / Volunteer / NGO",
  "Sportsperson",
  "Travel Professional",
  "Singer",
  "Writer",
  "Politician",
  "Associate",
  "Builder",
  "Chemist",
  "CNC Operator",
  "Distributor",
  "Driver",
  "Freelancer",
  "Mechanic",
  "Musician",
  "Photo / Videographer",
  "Surveyor",
  "Tailor",
  "Others",
  "Law Enforcement Officer",
  "Police",
  "Scientist / Researcher",
  "CXO / President / Director / Chairman",
  "VP / AVP / GM / DGM / AGM"
]));

export const DEGREE_OPTIONS = [
  // ================= Doctorate =================
  "Ph.D. - Doctor of Philosophy",
  "DM - Doctor of Medicine",
  "MCh - Master of Chirurgiae",
  "Postdoctoral Fellow",
  "FNB - Fellow of National Board",

  // ================= Civil Services / Elite =================
  "IAS - Indian Administrative Service",
  "IPS - Indian Police Service",
  "IRS - Indian Revenue Service",
  "IES - Indian Engineering Services",
  "IFS - Indian Foreign Service",

  // ================= Professional / Finance =================
  "CA - Chartered Accountant",
  "CFA - Chartered Financial Analyst",
  "CS - Company Secretary",
  "ICWA - Cost and Works Accountant",

  // ================= Post Graduate / Master's =================
  "MBA - Master of Business Administration",
  "PGDM - Post Graduate Diploma in Management",
  "M.Tech. - Master of Technology",
  "M.E. - Master of Engineering",
  "MCA - Master of Computer Applications",
  "M.Arch. - Master of Architecture",
  "M.Sc. - Master of Science",
  "M.Sc. IT/CS - Master of Science in IT/Computer Science",
  "M.S. Eng. - Master of Science in Engineering",
  "M.Com. - Master of Commerce",
  "M.A. - Master of Arts",
  "M.Ed. - Master of Education",
  "MDS - Master of Dental Surgery",
  "MD - Doctor of Medicine",
  "MS - Master of Surgery",
  "LLM - Master of Laws",
  "M.Pharm - Master of Pharmacy",
  "MPT - Master of Physiotherapy",
  "MHA - Master of Hospital Administration",
  "MHM - Master of Hotel Management",
  "MFM - Master of Financial Management",
  "MHRM - Master of Human Resource Management",
  "MLIS - Master of Library and Information Science",
  "MFA - Master of Fine Arts",
  "M.Phil. - Master of Philosophy",
  "MVSc - Master of Veterinary Science",
  "PGDCA - Post Graduate Diploma in Computer Applications",
  "Other Master's Degree in Engineering / Computers",
  "Other Master's Degree in Arts / Science / Commerce",

  // ================= Graduate / Bachelor's =================
  "MBBS - Bachelor of Medicine, Bachelor of Surgery",
  "B.Tech. - Bachelor of Technology",
  "B.E. - Bachelor of Engineering",
  "BCA - Bachelor of Computer Applications",
  "B.Arch. - Bachelor of Architecture",
  "B.Plan - Bachelor of Planning",
  "B.Sc. - Bachelor of Science",
  "B.Sc. IT/CS - Bachelor of Science in IT/Computer Science",
  "B.S. Eng. - Bachelor of Science in Engineering",
  "B.Com. - Bachelor of Commerce",
  "B.A. - Bachelor of Arts",
  "B.Ed. - Bachelor of Education",
  "BBA - Bachelor of Business Administration",
  "LLB - Bachelor of Legislative Law",
  "B.Pharm - Bachelor of Pharmacy",
  "BPT - Bachelor of Physiotherapy",
  "B.Sc. Nursing - Bachelor of Science in Nursing",
  "BDS - Bachelor of Dental Surgery",
  "BAMS - Bachelor of Ayurvedic Medicine and Surgery",
  "BHMS - Bachelor of Homeopathic Medicine and Surgery",
  "BSMS - Bachelor of Siddha Medicine and Surgery",
  "BUMS - Bachelor of Unani Medicine and Surgery",
  "BVSc - Bachelor of Veterinary Science",
  "BHM - Bachelor of Hotel Management",
  "BHA - Bachelor of Hospital Administration",
  "BFM - Bachelor of Financial Management",
  "BLIS - Bachelor of Library and Information Science",
  "BFA - Bachelor of Fine Arts",
  "BFT - Bachelor of Fashion Technology",
  "B.M.M. - Bachelor of Mass Media",
  "B.Phil. - Bachelor of Philosophy",
  "B.S.W. - Bachelor of Social Work",
  "Aeronautical Engineering",
  "Aviation Degree",
  "Other Bachelor's Degree in Engineering / Computers",
  "Other Bachelor's Degree in Arts / Science / Commerce",
  "Other Bachelor's Degree in Management",

  // ================= Diploma / Certifications =================
  "Diploma",
  "Polytechnic",
  "Other Diplomas",

  // ================= School Levels =================
  "12th Pass - Higher Secondary",
  "Class 1-5 - Primary School",
  "Class 6-10 - Secondary School",
];

export const INCOME_SLABS = [
  "₹ 1 Lakh & below",
  "₹ 1 Lakh to 2 Lakhs",
  "₹ 2 Lakhs to 3 Lakhs",
  "₹ 3 Lakhs to 4 Lakhs",
  "₹ 4 Lakhs to 5 Lakhs",
  "₹ 5 Lakhs to 6 Lakhs",
  "₹ 6 Lakhs to 7 Lakhs",
  "₹ 7 Lakhs to 8 Lakhs",
  "₹ 8 Lakhs to 9 Lakhs",
  "₹ 9 Lakhs to 10 Lakhs",
  "₹ 10 Lakhs to 12 Lakhs",
  "₹ 12 Lakhs to 14 Lakhs",
  "₹ 14 Lakhs to 16 Lakhs",
  "₹ 16 Lakhs to 18 Lakhs",
  "₹ 18 Lakhs to 20 Lakhs",
  "₹ 20 Lakhs to 25 Lakhs",
  "₹ 25 Lakhs to 30 Lakhs",
  "₹ 30 Lakhs to 35 Lakhs",
  "₹ 35 Lakhs to 40 Lakhs",
  "₹ 40 Lakhs to 45 Lakhs",
  "₹ 45 Lakhs to 50 Lakhs",
  "₹ 50 Lakhs to 60 Lakhs",
  "₹ 60 Lakhs to 70 Lakhs",
  "₹ 70 Lakhs to 80 Lakhs",
  "₹ 80 Lakhs to 90 Lakhs",
  "₹ 90 Lakhs to 1 Crore",
  "₹ 1 Crore & above",
];

// app/lib/constants.js

export const RASHI_OPTIONS = [
  "Mesh (Aries)",
  "Vrishabha (Taurus)",
  "Mithuna (Gemini)",
  "Karka (Cancer)",
  "Simha (Leo)",
  "Kanya (Virgo)",
  "Tula (Libra)",
  "Vrishchika (Scorpio)",
  "Dhanu (Sagittarius)",
  "Makara (Capricorn)",
  "Kumbha (Aquarius)",
  "Meena (Pisces)",
];

export const NAKSHATRA_OPTIONS = [
  "Ashwini",
  "Bharani",
  "Krittika",
  "Rohini",
  "Mrigashira",
  "Ardra",
  "Punarvasu",
  "Pushya",
  "Ashlesha",
  "Magha",
  "Purva Phalguni",
  "Uttara Phalguni",
  "Hasta",
  "Chitra",
  "Swati",
  "Vishakha",
  "Anuradha",
  "Jyeshtha",
  "Mula",
  "Purva Ashadha",
  "Uttara Ashadha",
  "Shravana",
  "Dhanishta",
  "Shatabhisha",
  "Purva Bhadrapada",
  "Uttara Bhadrapada",
  "Revati",
];

export const RELIGION_OPTIONS = [
  "Hindu",
  "Muslim - Shia",
  "Muslim - Sunni",
  "Muslim - Others",
  "Christian",
  "Sikh",
  "Jain - Digambar",
  "Jain - Shwetambar",
  "Jain - Others",
  "Parsi",
  "Buddhist",
  "Jewish",
  "Inter-Religion",
  "Other",
];

export const HEIGHT_RANGE_OPTIONS = [
  { label: "4'6\" - 5'0\" (137-152 cm)", value: "137-152" },
  { label: "5'0\" - 5'3\" (152-160 cm)", value: "152-160" },
  { label: "5'3\" - 5'6\" (160-167 cm)", value: "160-167" },
  { label: "5'6\" - 5'9\" (167-175 cm)", value: "167-175" },
  { label: "5'9\" - 6'0\" (175-182 cm)", value: "175-182" },
  { label: "6'0\" - 6'3\" (182-190 cm)", value: "182-190" },
  { label: "6'3\" - 7'0\" (190-213 cm)", value: "190-213" },
];

export const HEIGHT_OPTIONS = [
  { label: "4 ft 6 in (137 cm)", value: 137 },
  { label: "4 ft 7 in (139 cm)", value: 139 },
  { label: "4 ft 8 in (142 cm)", value: 142 },
  { label: "4 ft 9 in (144 cm)", value: 144 },
  { label: "4 ft 10 in (147 cm)", value: 147 },
  { label: "4 ft 11 in (149 cm)", value: 149 },
  { label: "5 ft 0 in (152 cm)", value: 152 },
  { label: "5 ft 1 in (154 cm)", value: 154 },
  { label: "5 ft 2 in (157 cm)", value: 157 },
  { label: "5 ft 3 in (160 cm)", value: 160 },
  { label: "5 ft 4 in (162 cm)", value: 162 },
  { label: "5 ft 5 in (165 cm)", value: 165 },
  { label: "5 ft 6 in (167 cm)", value: 167 },
  { label: "5 ft 7 in (170 cm)", value: 170 },
  { label: "5 ft 8 in (172 cm)", value: 172 },
  { label: "5 ft 9 in (175 cm)", value: 175 },
  { label: "5 ft 10 in (177 cm)", value: 177 },
  { label: "5 ft 11 in (180 cm)", value: 180 },
  { label: "6 ft 0 in (182 cm)", value: 182 },
  { label: "6 ft 1 in (185 cm)", value: 185 },
  { label: "6 ft 2 in (187 cm)", value: 187 },
  { label: "6 ft 3 in (190 cm)", value: 190 },
  { label: "6 ft 4 in (193 cm)", value: 193 },
  { label: "6 ft 5 in (195 cm)", value: 195 },
  { label: "6 ft 6 in (198 cm)", value: 198 },
  { label: "6 ft 7 in (200 cm)", value: 200 },
  { label: "6 ft 8 in (203 cm)", value: 203 },
  { label: "6 ft 9 in (205 cm)", value: 205 },
  { label: "6 ft 10 in (208 cm)", value: 208 },
  { label: "6 ft 11 in (210 cm)", value: 210 },
  { label: "7 ft 0 in (213 cm)", value: 213 },
];

export const FAMILY_STATUS_OPTIONS = [
  "Middle class",
  "Upper middle class",
  "Rich/Affluent(Elite)",
];

export const PHYSICAL_STATUS_OPTIONS = ["Normal", "Physically Challenged"];

export const allTongues = [
  "Telugu",
  "Tamil",
  "Hindi",
  "Malayalam",
  "Kannada",
  "Bengali",
  "Marathi",
  "Gujarati",
  "Punjabi",
  "Oriya",
  "Sindhi",
  "Urdu",
  "Assamese",
  "Bihari",
  "Bhojpuri",
  "Dogri",
  "English",
  "Garhwali",
  "Haryanvi",
  "Himachali / Pahari",
  "Kashmiri",
  "Konkani",
  "Kumaoni",
  "Kutchi",
  "Ladacki",
  "Magahi",
  "Maithili",
  "Marwari",
  "Nepali",
  "Rajasthani",
  "Sanskrit",
  "Santhali",
  "Sindhi",
  "Tulu",
  "Angika",
  "Arunachali",
  "Awadhi",
  "Badaga",
  "Bagri Rajasthani",
  "Brij",
  "Chatisgarhi",
  "Dhundhari / Jaipuri",
  "Garo",
  "Gujari / Gojari",
  "Harauti",
  "Kanauji",
  "Khandesi",
  "Khasi",
  "Koshali",
  "Lambadi",
  "Lepcha",
  "Malvi",
  "Manipuri",
  "Mewari",
  "Mewati / Ahirwati",
  "Miji",
  "Mizo",
  "Monpa",
  "Nicobarese",
  "Nimadi",
  "Shekhawati",
  "Sourashtra",
  "Tripuri",
  "Wagdi",
];

export const allCommunities = [
  "Brahmin",
  "Reddy",
  "Kamma",
  "Kapu",
  "Arya Vysya",
  "Mudaliar",
  "Gounder",
  "Vanniyar",
  "Chettiar",
  "Nair",
  "Ezhava",
  "Lingayat",
  "Vokkaliga",
  "Billava",
];

export const CASTES_HINDU = [
  "Don't wish to specify",
  "Caste no bar",
  "Other",

  "24 Manai Telugu Chettiar",
  "96 Kuli Maratha",
  "Aaru Nattu Vellala",
  "Achirapakkam Chettiar",
  "Ad Dharmi",
  "Adi Andhra",
  "Adi Dravidar",
  "Adi Karnataka",
  "Agamudayar / Arcot / Thuluva Vellala",
  "Agaram Vellan Chettiar",
  "Agarwal",
  "Agnikula Kshatriya",
  "Agrahari",
  "Agri",
  "Ahir",
  "Ahom",
  "Ambalavasi",
  "Anjana (Chowdary) Patel",
  "Arora",
  "Arunthathiyar",
  "Arya Vysya",
  "Badaga",
  "Baidya",
  "Balai",
  "Balija",
  "Balija Naidu",
  "Balija Reddy",
  "Baniya",
  "Banjara",
  "Bari",
  "Barnwal",
  "Besta",
  "Bhandari",
  "Bhatia",
  "Bhatraju",
  "Bhavasar Kshatriya",
  "Bhil",
  "Bhoi",
  "Bhovi",
  "Billava",
  "Bind",
  "Bishnoi",
  "Bondili",
  "Boyar",
  "Brahmin",
  "Brahmin - Anavil",
  "Brahmin - Audichya",
  "Brahmin - Barendra",
  "Brahmin - Bhatt",
  "Brahmin - Bhumihar",
  "Brahmin - Daivadnya",
  "Brahmin - Danua",
  "Brahmin - Deshastha",
  "Brahmin - Dhiman",
  "Brahmin - Dravida",
  "Brahmin - Gaur",
  "Brahmin - Havyaka",
  "Brahmin - Hoysala",
  "Brahmin - Iyengar",
  "Brahmin - Iyer",
  "Brahmin - Kanyakubj",
  "Brahmin - Karhade",
  "Brahmin - Kokanastha",
  "Brahmin - Madhwa",
  "Brahmin - Maithil",
  "Brahmin - Namboodiri",
  "Brahmin - Niyogi",
  "Brahmin - Rigvedi",
  "Brahmin - Saraswat",
  "Brahmin - Smartha",
  "Brahmin - Sri Vaishnava",
  "Brahmin - Vaidiki",
  "Brahmin - Velanadu",
  "Bunt",
  "CKP",
  "Chambhar",
  "Chandravanshi Kahar",
  "Chasa",
  "Chaudary",
  "Chaurasia",
  "Chettiar",
  "Chhetri",
  "Choudhary",
  "Coorgi",
  "Devadiga",
  "Devandra Kula Vellalar",
  "Devanga",
  "Devar / Thevar / Mukkulathor",
  "Dhakad",
  "Dhangar",
  "Dheevara",
  "Dhobi",
  "Ediga",
  "Ezhava",
  "Gabit",
  "Gandla",
  "Ganiga",
  "Garhwali",
  "Gavara",
  "Ghumar",
  "Goala",
  "Gomantak",
  "Gond",
  "Gopal",
  "Goud",
  "Gounder",
  "Gowda",
  "Gramani",
  "Gudia",
  "Gujjar",
  "Gupta",
  "Gurav",
  "Halba Koshti",
  "Heggade",
  "Holar",
  "Hugar",
  "Illaththu Pillai",
  "Jaiswal",
  "Jangam",
  "Jat",
  "Jatav",
  "Jetty",
  "Jhadav",
  "Jogi",
  "Kachara",
  "Kadia",
  "Kahar",
  "Kaibarta",
  "Kalal",
  "Kalinga",
  "Kalita",
  "Kalwar",
  "Kamboj",
  "Kamma",
  "Kapu",
  "Karana",
  "Karmakar",
  "Karuneegar",
  "Kasar",
  "Kashyap",
  "Kayastha",
  "Khandayat",
  "Khandelwal",
  "Kharwar",
  "Khatik",
  "Khatri",
  "Kirar",
  "Kodava",
  "Koli",
  "Kongu Vellala Gounder",
  "Konkani",
  "Kori",
  "Kosthi",
  "Kshatriya",
  "Kudumbi",
  "Kulal",
  "Kulalar",
  "Kumbhar",
  "Kumhar",
  "Kummari",
  "Kunbi",
  "Kurmi",
  "Kuruba",
  "Kurumbar",
  "Kushwaha",
  "Kutchi",
  "Lambadi",
  "Leva Patel",
  "Lingayat",
  "Lodhi Rajput",
  "Lohana",
  "Lohar",
  "Lubana",
  "Madiga",
  "Mahajan",
  "Mahar",
  "Maheshwari",
  "Mahishya",
  "Mala",
  "Mali",
  "Mallah",
  "Maratha",
  "Maruthuvar",
  "Matang",
  "Maurya",
  "Meena",
  "Meenavar",
  "Meghwal",
  "Meru Darji",
  "Mochi",
  "Mudaliyar",
  "Mudiraj",
  "Munnuru Kapu",
  "Nadar",
  "Naicker",
  "Naidu",
  "Nair",
  "Namasudra",
  "Nambiar",
  "Napit",
  "Nattukottai Chettiar",
  "Nayaka",
  "Nepali",
  "Nhavi",
  "Oraon",
  "Oswal",
  "Padmasali",
  "Pallan",
  "Panchal",
  "Pandaram",
  "Panicker",
  "Parkavakulam",
  "Parsi",
  "Patel",
  "Patil",
  "Pattinavar",
  "Perika",
  "Pillai",
  "Porwal",
  "Prajapati",
  "Pulaya",
  "Rajaka",
  "Rajbhar",
  "Rajbonshi",
  "Rajput",
  "Raju",
  "Ramoshi",
  "Rastogi",
  "Rathi",
  "Rawat",
  "Reddy",
  "Rohit",
  "Sadgope",
  "Saha",
  "Sahu",
  "Saini",
  "Saliya",
  "Sambava",
  "Sathwara",
  "Savji",
  "Sengunthar",
  "Settibalija",
  "Shah",
  "Shilpkar",
  "Shimpi",
  "Sindhi",
  "Sonar",
  "Soni",
  "Sourashtra",
  "Sugali",
  "Sunari",
  "Sundhi",
  "Suthar",
  "Swarnakar",
  "Tamboli",
  "Tanti",
  "Telaga",
  "Teli",
  "Thakur",
  "Thiyya",
  "Tili",
  "Turupu Kapu",
  "Urs",
  "Vaddera",
  "Vaish",
  "Vaishnav",
  "Vaishya",
  "Valmiki",
  "Vania",
  "Vanjari",
  "Vannar",
  "Vannia Kula Kshatriyar",
  "Variar",
  "Varshney",
  "Veera Saivam",
  "Velama",
  "Vellalar",
  "Vettuva Gounder",
  "Viswabrahmin",
  "Vishwakarma",
  "Vokkaliga",
  "Vysya",
  "Yadav",
];

export const CASTES_MUSLIM_SHIA = [
  "Muslim - Ansari",
  "Muslim - Arain",
  "Muslim - Awan",
  "Muslim - Bohra",
  "Muslim - Dekkani",
  "Muslim - Dudekula",
  "Muslim - Hanafi",
  "Muslim - Jat",
  "Muslim - Khoja",
  "Muslim - Lebbai",
  "Muslim - Malik",
  "Muslim - Mapila",
  "Muslim - Maraicar",
  "Muslim - Memon",
  "Muslim - Mughal",
  "Muslim - Others",
  "Muslim - Pathan",
  "Muslim - Qureshi",
  "Muslim - Rajput",
  "Muslim - Rowther",
  "Muslim - Shafi",
  "Muslim - Sheikh",
  "Muslim - Siddiqui",
  "Muslim - Syed",
  "Muslim - Unspecified",
  "Other Caste",
  "Don't wish to specify",
];

export const CASTES_MUSLIM_SUNNI = [
  "Muslim - Ansari",
  "Muslim - Arain",
  "Muslim - Awan",
  "Muslim - Bohra",
  "Muslim - Dekkani",
  "Muslim - Dudekula",
  "Muslim - Hanafi",
  "Muslim - Jat",
  "Muslim - Khoja",
  "Muslim - Lebbai",
  "Muslim - Malik",
  "Muslim - Mapila",
  "Muslim - Maraicar",
  "Muslim - Memon",
  "Muslim - Mughal",
  "Muslim - Others",
  "Muslim - Pathan",
  "Muslim - Qureshi",
  "Muslim - Rajput",
  "Muslim - Rowther",
  "Muslim - Shafi",
  "Muslim - Sheikh",
  "Muslim - Siddiqui",
  "Muslim - Syed",
  "Muslim - Unspecified",
  "Other Caste",
  "Don't wish to specify",
];
export const CASTES_CHRISTIAN = [
  "Don't wish to specify",
  "Adventist",
  "Anglican / Episcopal",
  "Anglo-Indian",
  "Apostolic",
  "Assembly of God (AG)",
  "Baptis",
  "Born Again",
  "Brethren",
  "Calvinist",
  "Chaldean Syrian (Assyrian)",
  "Christian - Others",
  "Church of Christ",
  "Church of God",
  "Church of North India",
  "Church of South India",
  "Congregational",
  "Evangelist",
  "Jacobite",
  "Jehovah's Witnesses",
  "Knanaya",
  "Knanaya Catholic",
  "Knanaya Jacobite",
  "Latin Catholic",
  "Latter day saints",
  "Lutheran",
  "Malabar Independent Syrian Church",
  "Malankara Catholic",
  "Marthoma",
  "Melkite",
  "Mennonite",
  "Methodist",
  "Moravian",
  "Orthodox",
  "Pentecost",
  "Presbyterian",
  "Protestant",
  "Reformed Baptist",
  "Reformed Presbyterian",
  "Roman Catholic",
  "Seventh-day Adventist",
  "St. Thomas Evangelical",
  "Syrian Catholic",
  "Syrian Jacobite",
  "Syrian Orthodox",
  "Syro Malabar",
];

export const CASTES_SIKH = [
  "Don't wish to specify",
  "Sikh - Ahluwalia",
  "Sikh - Arora",
  "Sikh - Bhatia",
  "Sikh - Ghumar",
  "Sikh - Intercaste",
  "Sikh - Jat",
  "Sikh - Kamboj",
  "Sikh - Khatri",
  "Sikh - Kshatriya",
  "Sikh - Lubana",
  "Sikh - Majabi",
  "Sikh - Others",
  "Sikh - Rajput",
  "Sikh - Ramdasia",
  "Sikh - Ramgharia",
  "Sikh - Saini",
  "Sikh - Unspecified",
  "Sikh - Ravidasia",
  "Sikh - Bhatra",
  "Sikh - Tonk Kshatriya",
  "Sikh - Nai",
  "Sikh - Rai",
];
export const CASTES_JAIN = [
  "Don't wish to specify",
  "Jain - Agarwal",
  "Jain - Bania",
  "Jain - Intercaste",
  "Jain - Jaiswal",
  "Jain - Khandelwal",
  "Jain - Kutchi",
  "Jain - Oswal",
  "Jain - Others",
  "Jain - Porwal",
  "Jain - Unspecified",
  "Jain - Vaishya",
  "Jain - KVO",
  "Jain - Asati",
  "Jain - Ayodhyavasi",
  "Jain - Bagherwal",
  "Jain - Barhiya",
  "Jain - Charanagare",
  "Jain - Chaturtha",
  "Jain - Dhakada",
  "Jain - Gahoi / Grihapati",
  "Jain - Golalare / Kharaua",
  "Jain - Golapurva",
  "Jain - Golsinghare",
  "Jain - Harada",
  "Jain - Humad / Humbad",
  "Jain - Kambhoja",
  "Jain - Kasar",
  "Jain - Kathanere",
  "Jain - Kharaua",
  "Jain - Lamechu",
  "Jain - Nema",
  "Jain - Padmavati Porwal",
  "Jain - Palliwal",
  "Jain - Panchama",
  "Jain - Parmar",
  "Jain - Parwar / Paravara",
  "Jain - Porwad / Porwal",
  "Jain - Saitwal",
  "Jain - Samanar / Nayinar",
  "Jain - Samiya",
  "Jain - Sarak",
  "Jain - Shrimal",
  "Jain - Upadhyaya",
  "Jain - Veerwal",
];
export const CASTES_PARSI = [
  "Don't wish to specify",
  "Intercaste",
  "Irani",
  "Parsi",
];
export const CASTES_BUDDHIST = [
  "Don't wish to specify",
  "Neo Buddhist",
  "Tibetan Buddhist",
];

export const CASTES_INTER_RELIGION = [
  "Don't wish to specify",
  "Ad Dharmi",
  "Adi Andhra",
  "Adi Dravidar",
  "Adi Karnataka",
  "Agarwal",
  "Agnikula Kshatriya",
  "Agri",
  "Ahir Shimpi",
  "Ahom",
  "Ambalavasi",
  "Arekatica",
  "Arora",
  "Arunthathiyar",
  "Arya Vysya",
  "Ayyaraka",
  "Badaga",
  "Baidya",
  "Baishnab",
  "Baishya",
  "Balija",
  "Banik",
  "Baniya",
  "Banjara",
  "Barai",
  "Bari",
  "Barujibi",
  "Besta",
  "Bhandari",
  "Bhatia",
  "Bhatraju",
  "Bhavasar Kshatriya",
  "Bhovi",
  "Billava",
  "Bishnoi / Vishnoi",
  "Born Again",
  "Boyar",
  "Brahmbatt",
  "Brahmin - Anavil",
  "Brahmin - Audichya",
  "Brahmin - Barendra",
  "Brahmin - Bhatt",
  "Brahmin - Bhumihar",
  "Brahmin - Daivadnya",
  "Brahmin - Danua",
  "Brahmin - Deshastha",
  "Brahmin - Dhiman",
  "Brahmin - Dravida",
  "Brahmin - Garhwali",
  "Brahmin - Gaur",
];

export const CASTES_BY_RELIGION = {
  Hindu: CASTES_HINDU,
  "Muslim - Shia": CASTES_MUSLIM_SHIA,
  "Muslim - Sunni": CASTES_MUSLIM_SUNNI,
  "Muslim - Others": CASTES_MUSLIM_SUNNI,
  Christian: CASTES_CHRISTIAN,
  Sikh: CASTES_SIKH,
  "Jain - Digambar": CASTES_JAIN,
  "Jain - Shwetambar": CASTES_JAIN,
  "Jain - Others": CASTES_JAIN,
  Parsi: CASTES_PARSI,
  Buddhist: CASTES_BUDDHIST,
  Jewish: CASTES_BUDDHIST,
  "Inter-Religion": CASTES_INTER_RELIGION,
};
