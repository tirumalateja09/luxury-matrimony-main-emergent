...
export default function AdminHomeSlidersPage() {
  ...
  const fetchSliders = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data?.message || 'Failed to fetch home sliders.');
      }
  
      setSliders(Array.isArray(data?.data) ? data.data : []);
    } catch (err) {
      setError(err?.message || 'Failed to fetch home sliders.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEditModal = (slider) => {
    ...
    setForm({
      image: slider?.image || '',
      isActive: typeof slider?.isActive === 'boolean' ? slider.isActive : true,
      order: typeof slider?.order === 'number' ? slider.order : Number(slider?.order || 0),
    });
    setIsModalOpen(true);
  };
  ...
  const uploadImages = useCallback(async (files) => {
    ...
    const urls = Array.isArray(photos) ? photos.map((photo) => photo?.url).filter(Boolean) : [];
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      ...
      toast.success('Homepage slider image uploaded successfully.');
    } catch (err) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const endpoint = isEdit
        ? `${process.env.NEXT_PUBLIC_API_URL}/home-sliders/${editingSlider._id}`
        : `${process.env.NEXT_PUBLIC_API_URL}/home-sliders`;
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.message || 'Failed to save homepage slider.');
      }

      toast.success(
        isEdit ? 'Homepage slider updated successfully.' : 'Homepage slider created successfully.'
      );
      closeModal();
      fetchSliders();
    } catch (err) {
      toast.error(err?.message || 'Failed to save homepage slider.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (sliderId) => {
    ...
    const confirmed = window.confirm('Are you sure you want to delete this homepage slider?');
    if (!confirmed) return;

    setDeletingId(sliderId);

    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/home-sliders/${sliderId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        ...
        throw new Error(data?.message || 'Failed to delete homepage slider.');
      }

      toast.success('Homepage slider deleted successfully.');
      setSliders((prev) => prev.filter((item) => item?._id !== sliderId));
    } catch (err) {
      toast.error(err?.message || 'Failed to delete homepage slider.');
    } finally {
      setDeletingId('');
    }
  };
        ...
        <div className='mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
          <div>
            ...
            <p className='text-gray-500'>
              Add, edit and remove homepage sliders from one place.
            </p>
          </div>
                ...
                <div className='flex items-center justify-between border-b border-[#F2E9DE] px-5 py-4'>
                  <div>
                    <h2 className='text-xl font-bold text-[#2D2424]'>
                      {editingSlider ? 'Edit Homepage Slider' : 'Add Homepage Slider'}
                    </h2>
                    <p className='text-sm text-gray-500'>
                      Fill in the homepage slider details below.
                    </p>
                  </div>
                  <button
                    type='button'
